...
export function CreateShipmentDialog({
  ...
  const [selectedCarrier, setSelectedCarrier] = useState<{code: string, name: string, label?: string} | null>(null);
  const [isSearchingCarrier, setIsSearchingCarrier] = useState(false);

  // Auto-detect carrier when tracking number changes
  const handleTrackingChange = (e: React.ChangeEvent<HTMLInputElement>, onChange: (value: string) => void) => {
      const value = e.target.value;
      onChange(value);

      // Only detect if carrier is not already manually selected
      if (!selectedCarrier && value.length > 5) {
      ...
      <DialogContent className="sm:max-w-[650px]">
        <DialogHeader className="space-y-1">
          ...
          <DialogDescription className="text-sm">
            Add a new shipment to track. Fill in the tracking number and customer details.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-3">
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input 
                        ...
                        {...field}
                        onChange={(e) => handleTrackingChange(e, field.onChange)}
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  <Popover open={openCarrierCombobox} onOpenChange={async (open) => {
                      setOpenCarrierCombobox(open);
                      if (open && carrierSearchResults.length === 0) {
                           const res = await searchCarriers('');
                           if (res.success && res.data) setCarrierSearchResults(res.data);
                      }
                  }}>
                          ...
                          >
                              <div className="flex items-center gap-2 truncate">
                                  ...
                                  <span className="truncate">
                                    {selectedCarrier ? selectedCarrier.name : "Auto-detect..."}
                                  </span>
                              </div>
                              {selectedCarrier && (
                                  <Badge variant="secondary" className="ml-2 text-xs">Set</Badge>
                              )}
                          </Button>
                      </PopoverTrigger>
                      ...
                      <PopoverTrigger asChild>
                          ...
                          >
                              <div className="flex items-center gap-2 truncate">
                                  ...
                                  <span className="truncate">
                                    {selectedUserId ? form.getValues('customer_name') : "Search user..."}
                                  </span>
                              </div>
                              {selectedUserId && (
                                  <Badge variant="secondary" className="ml-2 text-xs">Set</Badge>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="john@example.com" className="h-9" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="+1 234 567 8900" className="h-9" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
