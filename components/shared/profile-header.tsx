...
import { 
  ...
  Building2, Globe, ShieldCheck, User, BadgeCheck 
} from 'lucide-react';
...
export function ProfileHeader({ data, onAvatarUpload, uploading = false, editable = false }: ProfileHeaderProps) {
          ...
          <div className="flex-1 pt-16 sm:pt-4">
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-2 text-foreground/90 font-medium mb-4 text-sm">
                  ...
                  {data.joinedAt && (
                      <>
                        <span className="text-muted-foreground/40 text-lg font-light hidden sm:inline">|</span>
                        <div className="flex items-center gap-1.5 hover:text-primary transition-colors">
                            <Calendar className="h-3.5 w-3.5" />
                            <span>Joined {formatDistanceToNow(new Date(data.joinedAt), { addSuffix: true })}</span>
                        </div>
                      </>
                  )}
                  ...
                  {data.lastLogin && (
                      <>
                        ...
                        <div className="flex items-center gap-1.5 hover:text-primary transition-colors">
                            <Calendar className="h-3.5 w-3.5" />
                            <span>Last login {formatDistanceToNow(new Date(data.lastLogin), { addSuffix: true })}</span>
                        </div>
                      </>
                  )}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                 ...
                 {data.tenants && data.tenants.length > 0 && (
                    <Badge variant="outline" className="gap-2 pl-1.5 py-1 px-3 h-7 bg-background hover:bg-muted/50 transition-colors">
                         {data.tenants[0].countryCode && <CountryFlag countryCode={data.tenants[0].countryCode} className="h-3.5 w-5 rounded-[2px] shadow-sm" />}
                         <span className="font-medium text-foreground">{data.tenants[0].name}</span>
                    </Badge>
                 )}
                            ...
                            (
                                <span key="country" className="inline-flex items-center gap-1">
                                    {data.country && data.country.length === 2 && (
                                        <CountryFlag countryCode={data.country} className="h-2.5 w-4 rounded-[1px]" />
                                    )}
                                    {data.country}
                                </span>
                            )
