...
async function run() {
    ...
    if (fetchErr || !shipments || shipments.length === 0) {
        console.error('❌ No shipments found to test with. Run seed-mock-shipment.ts first.');
        return;
    }
    ...
    try {
        ...
        await notifier.notifyStatusChange({
            ...
            customerName: shipment.customer_details?.name || 'Test Customer',
            customerEmail: 'juddan2008@gmail.com', // Override for testing
