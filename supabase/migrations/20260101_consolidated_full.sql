-- ============================================================================
-- CONSOLIDATED MIGRATION - 2026-01-01
...
BEGIN
    ...
    END IF;
    -- Add to publication if not already present (safe check logic usually needed but assuming standard setup)
...
-- Logic: Email Notification on Shipment Status Change
...
BEGIN
  IF OLD.status IS DISTINCT FROM NEW.status AND NEW.status IN ('delivered', 'info_received', 'exception', 'out_for_delivery') THEN
    ...
    -- Check if email notification enabled for this tenant? (Optimistic)
...
-- Tenants: Users view their own tenants
CREATE POLICY tenants_select ON tenants FOR SELECT USING (id IN (SELECT tenant_id FROM user_tenants WHERE user_id = auth.uid()));
...
CREATE POLICY shipments_isolation ON shipments FOR ALL USING (tenant_id IN (SELECT tenant_id FROM user_tenants WHERE user_id = auth.uid()));

-- Leads: Tenant Isolation
CREATE POLICY leads_isolation ON leads FOR ALL USING (tenant_id IN (SELECT tenant_id FROM user_tenants WHERE user_id = auth.uid()));

-- Settings: Tenant Isolation
CREATE POLICY settings_select ON settings FOR SELECT USING (tenant_id IN (SELECT tenant_id FROM user_tenants WHERE user_id = auth.uid()));
CREATE POLICY settings_update ON settings FOR UPDATE USING (tenant_id IN (SELECT tenant_id FROM user_tenants WHERE user_id = auth.uid()) 
