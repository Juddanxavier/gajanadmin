...
import { CreateShipmentParams, ShipmentProvider, TrackingResult, ShipmentStatus, Carrier, TrackingCheckpoint } from '@/lib/tracking/types';
...
export class ShipmentService {
  ...
  private async getProvider(name: string = 'track123', tenantId?: string): Promise<ShipmentProvider> {
    switch (name) {
      case 'track123': {
  ...
  async createShipment(params: CreateShipmentParams & { userId?: string, invoiceDetails?: any, provider?: string }) {
    ...
    if (existing) {
        if (existing.deleted_at) {
            console.log(`[ShipmentService] Restoring soft-deleted shipment ${params.tracking_number}`);
            ...
            if (restoreError) throw new Error(`Failed to restore shipment: ${restoreError.message}`);
            return restored;
        } else {
            console.log(`[ShipmentService] Shipment ${params.tracking_number} already exists (active).`);
            throw new Error(`Tracking code ${params.tracking_number} already exists in the system.`);
        }
    }
    ...
    const { data: shipment, error } = await this.client
        ...
        .insert({
            ...
            carrier_id: trackingResult.carrier_code || params.carrier_code || 'unknown',
  ...
  async getShipments({ page = 1, pageSize = 10, filters = {}, sortBy }: GetShipmentsParams) {
    let query = this.client.from('shipments').select('*, carrier:carriers(name_en, name_cn)', { count: 'exact' });
    ...
    if (filters.search) query = query.ilike('carrier_tracking_code', `%${filters.search}%`);
  ...
  async syncShipment(shipmentId: string, overrideCarrierCode?: string) {
    ...
    const provider = await this.getProvider(shipment.provider, shipment.tenant_id);
    const carrierToUse = overrideCarrierCode || shipment.carrier_id;

    try {
        const result = await provider.getTracking(shipment.carrier_tracking_code, carrierToUse);
  ...
  async updateShipment(shipmentId: string, updates: Partial<CreateShipmentParams> & { status?: string }) {
      const updatePayload: any = {};
      if (updates.carrier_code) updatePayload.carrier_id = updates.carrier_code;
      if (updates.customer_name || updates.customer_email || updates.customer_phone) {
          ...
          if (shipment) {
              try {
                  ...
                  if (provider.stopTracking) {
                      await provider.stopTracking(shipment.carrier_tracking_code, shipment.carrier_id);
                  }
              } catch (e) {
                  console.warn(`[ShipmentService] Failed to stop tracking remotely for ${shipment.carrier_tracking_code}`, e);
  ...
  async getStats(filters: { tenantIds?: string[] } = {}) {
      let query = this.client.from('shipments').select('status, created_at, updated_at');
      ...
      const pending = data.filter(s => ['pending', 'created', 'info_received'].includes(s.status)).length;
      const in_transit = data.filter(s => ['in_transit', 'out_for_delivery'].includes(s.status)).length;
      
      const deliveredShipments = data.filter(s => s.status === 'delivered');
      const delivered = deliveredShipments.length;
      
      const exception = data.filter(s => ['exception', 'attempt_fail', 'expired'].includes(s.status)).length;
      ...
      if (delivered > 0) {
          ...
          avgDeliveryDays = Math.round((totalDurationMs / delivered) / (1000 * 60 * 60 * 24));
      }

      return { total, pending, in_transit, delivered, exception, avgDeliveryDays };
  }
  ...
  async searchCarriers(query: string = '') {
      ...
      // 1. DB Search
      let dbQuery = this.client.from('carriers').select('code, name_en, name_cn, homepage, logo_url');
      if (query.length >= 2) {
          dbQuery = dbQuery.or(`code.ilike.%${query}%,name_en.ilike.%${query}%,name_cn.ilike.%${query}%`);
      }
      ...
      // 2. API Fallback (simplified for now directly calling provider if needed)
       ...
       return apiCarriers
        .filter(c => !query || c.code.toLowerCase().includes(lowerQ) || c.name.toLowerCase().includes(lowerQ))
        .slice(0, 50);
  }
  ...
  async processWebhook(payload: any, tenantId?: string) {
    const { trackNo, transitStatus, transitSubStatus, localLogisticsInfo } = payload;
    ...
    if (!shipments || shipments.length === 0) {
      console.warn(`[ShipmentService] No shipment found for tracking number: ${trackNo}`);
      return null;
    }
    ...
    for (const shipment of shipments) {
      ...
      // Map checkpoints
      const checkpoints: TrackingCheckpoint[] = (localLogisticsInfo?.trackingDetails || []).map((detail: any) => ({
  ...
  private mapStatus(status: string): ShipmentStatus {
    ...
    if (upper?.includes('EXCEPTION') || upper?.includes('RETURN')) return 'exception';
  ...
  // Private helpers
  private async updateShipmentFromTrackingResult(shipmentId: string, result: TrackingResult) {
    const { data: existing } = await this.client
        .from('shipments')
        .select('*')
        .eq('id', shipmentId)
