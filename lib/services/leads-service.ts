...
export class LeadsService {
    ...
    await Promise.all(
      allUserIds.map(async (uid) => {
        try {
          const {
            data: { user },
          } = await adminClient.auth.admin.getUserById(uid as string);
          if (user) {
            userMap.set(uid, {
              email: user.email || '',
              name:
  ...
  async getLead(id: string, tenantIds?: string[]): Promise<Lead | null> {
    ...
    try {
      const [cResult, aResult] = await Promise.all([
        ...
        lead.assigned_to
          ? adminClient.auth.admin.getUserById(lead.assigned_to)
      ...
      if (cResult.data.user) {
        customer = {
          ...
          name:
            cResult.data.user.user_metadata?.display_name ||
            cResult.data.user.user_metadata?.name ||
            cResult.data.user.email ||
            'Unknown',
          phone:
            cResult.data.user.phone ||
            cResult.data.user.user_metadata?.phone ||
            '',
        };
      }
      ...
      if (aResult.data.user) {
        assignee = {
          ...
          name:
            aResult.data.user.user_metadata?.display_name ||
            aResult.data.user.user_metadata?.name ||
            aResult.data.user.email ||
            'Unknown',
        };
      }
    } catch (err) {
  ...
  async getLeadTrends(days: number = 30, tenantIds?: string[]) {
    ...
    const trends: Record<
      string,
      { total: number; converted: number; lost: number; value: number }
    > = {};

    // Initialize all days (from oldest to newest)
    for (let i = days - 1; i >= 0; i--) {
      const d = new Date();
      d.setDate(d.getDate() - i);
      const dateStr = d.toISOString().split('T')[0];
      trends[dateStr] = { total: 0, converted: 0, lost: 0, value: 0 };
    }
    ...
    data?.forEach((l) => {
      ...
      if (trends[dateStr]) {
        ...
        trends[dateStr].value += l.value || 0;
        if (l.status === 'completed') trends[dateStr].converted++;
        if (l.status === 'failed' || l.status === 'deleted')
          trends[dateStr].lost++;
      }
    });
