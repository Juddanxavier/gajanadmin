import { ShipmentProvider, TrackingResult, CreateShipmentParams, ShipmentStatus, TrackingCheckpoint } from '../types';
...
export class Track123Provider implements ShipmentProvider {
  ...
  async createTracker(params: CreateShipmentParams): Promise<TrackingResult> {
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl.replace('v2', 'v2.1')}/track/import`, {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        throw new Error(`Track123 Import Error (${response.status}): ${errText}`);
      }
      ...
      if (rejected) {
        ...
        console.warn(`[Track123] Import rejected for ${params.tracking_number}. Trying to fetch details...`, rejected);
        try {
           return await this.getTracking(params.tracking_number, rejected.courierCode || params.carrier_code);
        } catch (e) {
           throw new Error(`Track123 Import Failed: ${JSON.stringify(rejected)} (Sync failed: ${e})`);
        }
      }
   ...
   */
  async stopTracking(tracking_number: string, carrier_code?: string): Promise<boolean> {
      ...
      try {
          ...
          const response = await fetch(`${this.baseUrl.replace('v2', 'v2.1')}/track/delete`, {
              method: 'POST',
              headers: this.getHeaders(),
              body: JSON.stringify(payload)
          });
          ...
          if (data?.code === 0 || data?.msg?.toLowerCase() === 'success') {
             console.log(`[Track123] Successfully deleted tracking for ${tracking_number}`);
             return true;
          }
   ...
   */
  async getTracking(tracking_number: string, carrier_code?: string, _retry = false): Promise<TrackingResult> {
    if (!this.apiKey) return this.mockResponse(tracking_number);

    try {
      // Treat 'unknown' as null to allow auto-detection or generic search
      const cleanCarrier = (carrier_code === 'unknown' || !carrier_code) ? null : carrier_code;
      ...
      if (!response.ok) throw new Error(`Track123 Query Error: ${response.statusText}`);

      const data = await response.json();
      
      // The V2 Query response is typically data.data.list[]
      const trackingData = data.data?.list?.[0] || data.data?.accepted?.content?.[0];
      ...
      if (!trackingData) {
         ...
         if (rejected) {
             ...
             if (errCode === 'A0400' && !_retry) {
                 console.log(`[Track123] Tracking ${tracking_number} not registered. Attempting auto-import...`);
                 
                 // Attempt Import
                 try {
                     await this.createTracker({ tracking_number, carrier_code: cleanCarrier || undefined });
                     // Retry Fetch
         ...
         throw new Error(`Track123 Get Error: Not Found (Request: ${tracking_number}/${cleanCarrier}, Response: ${JSON.stringify(data)})`);
      }
  ...
  private normalizeResponse(data: any): TrackingResult {
    ...
    // Debug logging
    console.log('[Track123] Normalizing response. Top-level keys:', Object.keys(data));
    ...
    if (logisticsInfo && logisticsInfo.trackingDetails && Array.isArray(logisticsInfo.trackingDetails)) {
        rawCheckpoints = logisticsInfo.trackingDetails;
    }

    const checkpoints: TrackingCheckpoint[] = rawCheckpoints.map((cp: any) => ({
      occurred_at: cp.created_at || cp.time || cp.trackingTime || cp.eventTime || new Date().toISOString(),
      location: cp.location || cp.city || cp.trackingLocation || cp.address,
      description: cp.message || cp.status_name || cp.trackingStatus || cp.eventDetail,
      status: this.mapStatus(cp.status || cp.trackingStatus || cp.transitSubStatus || cp.eventDetail),
    ...
    const carrierCode = t.carrier_code || data.courierCode || data.CourierCode || data.carrierId || logisticsInfo?.courierCode || '';
    
    // Status extraction: Prioritize transitStatus (most detailed) -> trackingStatus -> status
    let status = this.mapStatus(data.transitStatus || data.trackingStatus || data.status || data.trackStatus);
    ...
    if (logisticsInfo) {
      latestLocation = logisticsInfo.location || logisticsInfo.city || logisticsInfo.trackingLocation;
  ...
  async getCarriers(): Promise<import('../types').Carrier[]> {
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl.replace('v2', 'v2.1')}/courier/list`, {
        method: 'GET',
        headers: this.getHeaders()
      });

      if (!response.ok) throw new Error(`Track123 List Carriers Error: ${response.statusText}`);
  ...
  private mockResponse(tracking_number: string): TrackingResult {
    return {
      ...
      status: 'in_transit',
      checkpoints: [
          { occurred_at: new Date().toISOString(), status: 'in_transit', description: 'Mock Update', location: 'Test City' }
      ],
