...
import { ArrowLeft, MapPin, User, Phone, Mail, Package, CreditCard, Calendar, ShieldCheck } from "lucide-react";
...
export default function LeadDetailsPage() {
  ...
  const [teamMembers, setTeamMembers] = useState<UserDisplay[]>([]);
  ...
  const handleStatusChange = async (newStatus: string) => {
      ...
      if (result.success) {
          ...
          setLead(prev => prev ? { ...prev, status: newStatus as LeadStatus } : null);
      } else {
  ...
  const handleAssign = async (userId: string) => {
      ...
      const result = await assignLeadAction(lead.id, userId === "unassigned" ? null : userId);
      ...
      if (result.success) {
          ...
          setLead(prev => prev ? { 
              ...
              assignee: userId === "unassigned" ? undefined : {
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
             ...
             <div className="flex items-center gap-4">
                 <div className="flex items-center gap-2 bg-background border px-3 py-1.5 rounded-md shadow-sm">
                     ...
                     <Select disabled={isUpdating} value={lead.status} onValueChange={handleStatusChange}>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card className="hover:shadow-md transition-shadow border-green-500/20 bg-green-500/5">
               <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
                    <CardTitle className="text-sm font-medium text-green-600 dark:text-green-400">Total Value</CardTitle>
                    <CreditCard className="h-4 w-4 text-green-500" />
                </CardHeader>
                <CardContent>
                    <div className="text-2xl font-black text-green-700 dark:text-green-300">{formatCurrency(lead.value)}</div>
                </CardContent>
            </Card>
                ...
                <CardContent className="space-y-4">
                     ...
                     <div className="pt-2 space-y-3">
                         <div className="space-y-1">
                            ...
                            <Select disabled={isUpdating} value={lead.assigned_to || "unassigned"} onValueChange={handleAssign}>
