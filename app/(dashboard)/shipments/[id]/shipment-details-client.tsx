...
export function ShipmentDetailsClient({
  shipment,
  events,
  logs,
  ...
  // Format helpers
  const origin =
    raw.shipFrom || raw.ship_from || shipment.origin_country || 'Unknown';
  const destination =
    raw.shipTo ||
    raw.ship_to ||
  ...
  const originCountryCode =
    ...
    (origin && origin.length === 2 ? origin : undefined);
  const destCountryCode =
    shipment.destination_country ||
    (destination && destination.length === 2 ? destination : undefined);
  ...
  return (
    <div className='container mx-auto py-6 max-w-6xl space-y-8'>
      ...
      <div className='flex flex-col gap-8'>
        ...
        {/* Hero Route Card */}
        <div
          className={`relative overflow-hidden rounded-xl border bg-gradient-to-br ${currentGradient} p-8 duration-500 transition-colors`}>
          ...
          <Card className='hover:shadow-md transition-shadow'>
            ...
            <CardContent>
              <div
                ...
                title={shipment.white_label_code}>
                  ...
                  <div className='relative space-y-8'>
                    ...
                    {events.map((event, index) => (
                      <div key={event.id} className='relative flex gap-6'>
                        {/* Timeline dot */}
                        <div
                          className={`relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-4 border-background ${index === 0 ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
                          {index === 0 ? (
